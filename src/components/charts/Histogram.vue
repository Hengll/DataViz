...
const data = computed(() => {
  ...
  const bins = editor.dashboard.charts[props.indexOfChart].chartOption.histogram.bins
  const data = {}

  for (let i = 0; i < Variables0.length; i++) {
    const groupKey = Math.floor(Variables0[i] / bins) * bins
    if (!data[groupKey]) {
      data[groupKey] = Variables1[i] * 1
    } else {
      data[groupKey] += Variables1[i] * 1
    }
  }
...
const chartData = computed(() => {
  const bins = editor.dashboard.charts[props.indexOfChart].chartOption.histogram.bins
  return {
    labels: Object.keys(data.value).map((key) => `${key} - ${Number(key) + bins - 1}`),
    datasets: [
      {
...
const chartOptions = computed(() => {
  ...
  return {
    ...
    indexAxis: editor.dashboard.charts[props.indexOfChart].chartOption.histogram.indexAxis,
    layout: {
      padding: {
        left:
    ...
    elements: {
      bar: {
        ...
        borderWidth:
          ...
          props.gridWidth,
        borderColor:
          editor.dashboard.charts[props.indexOfChart].chartOption.histogram.histogramBorderColor,
        borderRadius: {
          topLeft:
            editor.dashboard.charts[props.indexOfChart].chartOption.histogram.histogramBorderRadius
              .topLeft,
          topRight:
            editor.dashboard.charts[props.indexOfChart].chartOption.histogram.histogramBorderRadius
              .topRight,
        ...
        inflateAmount:
          editor.dashboard.charts[props.indexOfChart].chartOption.histogram.histogramlateAmount,
      },
    },
