...
const variance = computed(() => {
  const Variables0 = editor.filterData.map(
    (row) => row[editor.dashboard.charts[props.indexOfChart].useVariables[0]] * 1,
  )

  if (Variables0.length === 0 || isNaN(Variables0[0] * 1)) return NaN

  const mean = Variables0.reduce((sum, num) => sum * 1 + num * 1, 0) / Variables0.length

  const squaredDiffs = Variables0.map((num) => (num - mean) ** 2)
  const varianceValue = squaredDiffs.reduce((sum, num) => sum + num, 0) / (Variables0.length - 1)
  const round = editor.dashboard.charts[props.indexOfChart].chartOption.typography.round

  return Math.round(Math.sqrt(varianceValue) * 10 ** round) / 10 ** round
})
